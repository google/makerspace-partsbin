...
module dogbone_finger_negative(material_thickness, bit_size, tab_width, hole=true, dogbones=3) {
  ...
  if (dogbones == 2 || dogbones == 3)
    ...
    circle(d=bit_size,$fn=32);
  if(hole)
    translate([0,material_thickness/2]) circle(d=bit_size,$fn=32);
}
...
function offset_events(ev, symmetric_value) = [
  for(i=[0:len(ev)-1])
    (i==0 ? ev[i] : i==len(events)-1 ? [ev[i][0], ev[i][1]+symmetric_value*2]
    : [ev[i][0], ev[i][1]+symmetric_value])
];
...
module generic_finger_negative(material_thickness, bit_size, events, hole=true) {
  for(i=[0:len(events)-1]) {
    if((events[i][0] == 1) && i < len(events)-1) { // begin notch
      ...
      translate([events[i][1]-bit_size/2,0]) circle(d=bit_size,$fn=32);
      if(hole)
...
module hook_finger_positive(material_thickness, bit_size, events, slide=8, a=0.5, t=8, r=1.5, u=0.2) {
  echo(events);
  for(i=[0:len(events)-1]) {
    if(events[i][0] == 1 && i < len(events)-1) {
      x1 = events[i][1]; x2 = events[i+1][1];
      offset(r=r) offset(delta=-r) polygon(points=[
        [x1, -r-1], [x1, material_thickness-u], [x1-slide*0.7, material_thickness-u],
        [x1-slide,material_thickness-u+a], [x1-slide, material_thickness+t],
        [x2-slide, material_thickness+t], [x2-slide, -r-1]], convexity=4);
    }
  }
}

module hook_finger_negative(material_thickness, bit_size, events, slide=8, a=0.5, t=8, u=0.2) {
  for(i=[0:len(events)-1]) {
    if((events[i][0] == 1) && i < len(events)-1) { // begin notch
      // can we always ignore last one, or need to know width?
      translate([events[i][1]-bit_size/2,0]) circle(d=bit_size);
      translate([events[i][1],material_thickness-u-bit_size/2]) circle(d=bit_size);
    } else if(events[i][0] == 0 && i > 0) {
